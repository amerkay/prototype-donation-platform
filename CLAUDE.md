...
| \_admin       | `app/features/_admin/`                 | Admin-specific UI: sidebar, data table, AdminListPage, AdminConfigPanel, AdminEntityCard, AdminCardGrid, PreviewEditable, quick find; admin detail cards (DonorInfoCard with stats, LineItemsCard, CustomFieldsCard, TransactionHistoryCard), `useDonorStats`                                     |
| \_shared      | `app/features/_shared/`                | Cross-boundary shared: BreadcrumbBar, EditLayout, useEditState, useEntityDataService, NavUser, column-builders, filter utils                                                                                                                                                                     |
...
26. **Detail cards split**: Admin has DonorInfoCard (with stats), LineItemsCard, CustomFieldsCard, TransactionHistoryCard. Portal has PortalDonorCard (no stats/link), PortalLineItemsCard. `_shared/` holds cross-boundary infra (EditLayout, useEditState, etc.).
