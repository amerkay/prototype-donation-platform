...
17. **COMMIT MESSAGE STANDARD**: Use short conventional commits following rule #15. NEVER include the "Authored by Anthropic" line in commit messages. Always check for all staged changes `git --no-pager diff --staged` before writing the commit message.
