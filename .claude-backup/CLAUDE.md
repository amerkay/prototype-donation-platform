...
23. **Currency totals**: `totalAmount × exchangeRate` = base currency; `totalAmount × campaignExchangeRate` = campaign currency. Campaign currency is immutable after creation.
...
34. **Campaign currency immutability**: `crowdfunding.currency` set at creation, hidden from edit form (`visibleWhen: !store.id`), store setter guards against changes.
